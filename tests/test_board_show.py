...
def test_a_jira_description_reads_as_text():
    ...
    ticket = _issue_to_ticket(
        {"key": "RT-1", "fields": {"summary": "s", "description": ADF}}
    )
    assert ticket.description == "Export invoices\n- Done when CSV downloads"
    assert (
        _issue_to_ticket({"key": "RT-2", "fields": {"description": None}}).description
        == ""
    )
...
def test_board_show_prints_the_description():
    ...
    with patch("rite_ai.cli.main._ticket_backend", return_value=(backend, None)):
        result = CliRunner().invoke(cli, ["board", "show", "RT-9"])
    assert result.exit_code == 0, result.output
    backend.read.assert_called_once_with("RT-9")
    assert "RT-9  [To Do]  Export invoices" in result.output
...
def test_board_show_says_why_it_could_not_read():
    ...
    backend = MagicMock()
    backend.read.return_value = BackendError("RT-404 → 404")
    with patch("rite_ai.cli.main._ticket_backend", return_value=(backend, None)):
        result = CliRunner().invoke(cli, ["board", "show", "RT-404"])
