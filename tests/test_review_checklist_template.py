...
def test_default_template_has_verification_category_with_named_lessons():
    ...
    assert len(verification_items) == 16
    ...
    assert "verification run is in flight" in joined  # tree moved mid-suite
    assert "which assertion catches it" in joined  # one proxy did the work
    assert "where the property has structure" in joined  # structure vs stance
    assert "specifies it" in joined  # a test that writes the defect down
    # a liveness assertion satisfied by the test process's own liveness, so it
    # holds with the logic deleted and asserts the bug as the feature
    assert "measuring its own fixture" in joined
