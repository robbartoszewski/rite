...
| K3 | **The queue is a draft — re-evaluated at window open** | At the first cycle boundary inside a window, the cycle instruction carries the queued questions and one directive: withdraw any you can now answer. `rite question withdraw <id> --answered-by <anchor>` — an anchor, through the journal's anchor floor, because "I worked it out" is not checkable. Only survivors are delivered. **The filter's value is counted**: queued / withdrawn-before-asking / asked, reported in every digest, so "deferral filters" is measured rather than believed. | Through `rite start`: two questions queued before a window; the re-evaluation cycle withdraws one with an anchor; only the other reaches the outbox and Slack; the digest reports 2 queued, 1 withdrawn, 1 asked. A withdrawal with no anchor is refused. **And once with a real Claude Manager** that has answered one of its own queued questions in the meantime: it withdraws that one and only that one. | K2 | 2–3 sittings |
