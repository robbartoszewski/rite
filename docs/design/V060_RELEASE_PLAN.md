...
| C13 | ✅ **DECIDED, `508349c`: `--manager` KEPT on purpose**, because tmux older than 3.2 gives a session no `RITE_MANAGER`. The reason is at `journal.instructions`. ⚠ Those instructions still say bare `rite journal …` rather than `own_command()`, the class C27 fixed for `rite reply`. **`journal.instructions()` still spells out `--manager`** | `V070_MULTI_MANAGER.md` §2 | Redundant since `RITE_MANAGER`; pinned by tests, so it waits for a deliberate change to that text. | ½ sitting |
