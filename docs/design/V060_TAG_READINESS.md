...
| D12 | **CI green** | ⏳ **A macOS job exists, on `c28/sandbox-backends`, NOT yet on `main`** (`boundary-and-credentials-on-macos`, `macos-latest`). It runs six named boundary and credential test files, not the whole suite: a hosted runner has no gitleaks and no credential store, so a full run would skip what it exists to check. It failed on its first runs (4 defects found, per the building session; run `36221810098` at `4c9b9ce` failed) and has passed since: runs `36222217489` (`ebe25ee`) and `36222869929` (`0cba16f`). Not green on `main` until it is merged. The full suite runs on macOS only locally (4572 passed, 29 skipped at `ee350c6`), which is not CI | ✅ **CI fully green on `main` at `691a248`** (run `36221689241`), the first time in this thread: Python 3.11 4563 passed / 38 skipped, 3.12 and 3.13 4562 / 39, 0 failed. C28 is closed. ✅ **The publish gate is GREEN at `691a248`** (run `36221689224`), CLOSED: it had failed on every push from at least `d122c2a` on 5 gitleaks findings in test fixtures, and another session suppressed all five in `.rite/gitleaksignore`, each with a reason pinned to its content (`6080140`), including this list's author's fake `ghs_` token. ⚠ **Known flaky under load, not a defect:** `test_manager_endings_and_resume.py::test_a_killed_manager_is_a_crash_not_an_ambiguity` failed once and passed 3/3 on rerun (reported by the session that saw it). A single failure of it on tag day is this, until it fails on a rerun | the macOS job merged to `main` and green there |
