...
- **Option 3** is the only one where you read the whole thing first. It is
  ...
  `curl | sh` in someone else's project. Its `git checkout v0.5.0` is the
...
curl -fsSLO https://raw.githubusercontent.com/robbartoszewski/rite/v0.5.0/install.sh
shasum -a 256 install.sh          # compare against the v0.5.0 release notes
sh install.sh

# 3. clone and read everything
git clone https://github.com/robbartoszewski/rite.git
cd rite && git checkout v0.5.0
