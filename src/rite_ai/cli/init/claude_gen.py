...
section with what it cites and the sections everything depends on — a fraction
of the document. If the spec has been digested, `rite spec show <unit>` prints
that section rewritten shorter and reviewed against its source; it exits
non-zero when what it prints no longer matches the spec. When that is not
enough and you read the whole spec anyway, record it: `rite handover write
--spec-fallback <unit>`. That count is the only evidence the slices are too
small, and a slice nobody reports on reads exactly like one that worked.

**Notes of your own go in `.rite/spec-notes.md`, not here.** This section is
regenerated from `.rite/` every time rite refreshes your files, so anything
written into it is replaced without warning. That file is never generated and
never rewritten — it is the one place near the spec that is yours.
...
`.rite/modules.yaml`, and `.rite/config.yaml`.

**It is refreshed after init, and that is the point.** `rite update
--files-only` brings the sections rite wrote up to date with a newer rite;
`--dry-run` shows what would change first. Sections you have edited are kept
and reported rather than overwritten — with one exception worth knowing:
`## Project spec` is regenerated in full on every refresh, so prose added
there does not survive. `rite spec add` and `rite spec remove` rewrite it too.

(This paragraph used to say "nothing else in it is rewritten after init",
which told every session and every reader not to expect the refresh that the
whole `rite update` feature exists to provide.)"""
