...
Default No — an added dependency should be opted into, not defaulted on. If
Yes, `sandbox.enabled: true` is written to `config.yaml`, and `rite add
worker` (§9.6) provisions each Worker's scoped git credential as part of
creating it — the questionnaire itself does not collect or generate any token.
...
| D-51 | Sandbox default | **Opt-in: `sandbox.enabled: false`, and the §9.3 questionnaire defaults the answer to No** | "An added dependency should be opted into, not defaulted on" (§9.3 Section 6). yoloAI is a separate binary from yoloai.dev, not a declared dependency in `pyproject.toml`, so defaulting on would fail first run for everyone without it. Recorded here rather than left in questionnaire prose because it is a posture decision about a security feature gated on an external dependency, not a tuning value. **Not a precedent for migrating defaults into this register:** numeric defaults keep their rationale in `config/models.py` beside the value, which is where someone changing one will read it. |
