...
| D12 | **CI green** | **No macOS CI exists.** The suite is run locally only (4572 passed, 29 skipped at `ee350c6`), which is not CI | ✅ **CI green on `ubuntu-latest`, Python 3.11/3.12/3.13**, at `b3c14c7`: 4563 passed, 38 skipped on one Python, 4562 passed, 39 skipped on the other two, 0 failed. C28 is closed. ❌ **The publish gate is RED** on every push since at least `d122c2a` (six checked): 5 gitleaks findings in test fixtures, one of them this list's author's fake `ghs_` token (`f3926a1`). Another session is fixing it | publish gate green; a decision on macOS CI |
...
**And not with the publish gate red (D12).** It has failed on every recent
push, so nothing on `main` passes the gate a release goes through.
