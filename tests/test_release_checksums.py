...
def test_it_refuses_to_publish_a_digest_of_an_uncommitted_working_copy(tmp_path):
    ...
    assert "no v0.1.0 tag exists yet" in proc.stderr
...
def _tagged_clone(tmp_path: Path, tag: str = "v0.1.0") -> Path:
...
def test_it_hashes_the_tag_not_head_once_the_tag_exists(tmp_path):
    ...
    at_tag = subprocess.run(
        ["git", "show", "v0.1.0:install.sh"], cwd=clone, capture_output=True, check=True
    ).stdout
    ...
    assert "tag v0.1.0" in out.stdout
