...
*Since: MMQ2 is decided (option (c)) and built, so only the Owner hears
Slack (`e18139d`, SPEC §9.16.7). MMQ5 ships as built, and its question is
v0.7.0's.*
...
are v0.7.0; memory stays v0.7.0. *Re-filed 2026-09-26: multi-Manager on one
machine is in v0.6.0 (the banner above), across machines it is v0.8.0;
Cursor stays v0.7.0; memory is in no release yet (`V070_RELEASE_PLAN.md`,
top).*
...
|---|---|---|---|---|---|
| A1 | ✅ **LANDED, `03d1af6`** (a cursor per reader, Decision 1(a); retention as C23, `b4cc746`). **Per-reader delivery for the outbox** | Today a reader is told *"delete one once you have relayed it so it is not shown twice."* With two permanent readers, whoever reads first deletes and the other never sees it. Needs per-reader read state, fan-out, or acknowledgement. | **Blocks Slack outright.** Robert's "both keep working permanently" requirement and the current delete-on-relay instruction cannot both hold. | Decision 1 | 1–2 sittings |
| A2 | ✅ **LANDED, `1fa9b5e`** (the store has since become one 0600 file, `a732682`). **Slack credentials through the existing store** | A bot token is a credential; `credentials/` and the gate already have the shapes. | A token is the first thing the adapter needs and the last thing that should be improvised. | — | 1 sitting |
...
destination control, v0.7.0 (*v0.8.0 since the re-filing of 2026-09-26*), [`V070_EGRESS.md`](V070_EGRESS.md): the agent
...
of these are urgent, and that is noted per row. *(Multi-Manager on one
machine moved back into v0.6.0 on 2026-09-26.)*
