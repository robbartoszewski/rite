...
| D16 | **A Goose Manager starts on a FRESH machine** | ✅ seatbelt grants a path that does not exist yet | ❌ **obs, open:** `landlock.py:277` grants Goose's state directories only `if p.exists()`, and Goose must also create `~/.config/goose`. On a machine where Goose never ran, the first start panics: `Failed to create session database directory: … Permission denied`, then `Failed to create config directory`. A platform DIVERGENCE: a Landlock rule names an inode, so it cannot grant what does not exist yet. ⚠ **The VM no longer shows it:** both directories were created by hand. To reproduce, move `~/.local/share/goose` and `~/.config/goose` aside (recipe in `~/RITE-TEST-BOX-INSTALLED.txt`; verified to reproduce) | the `landlock.py` owner: create the engine's directories before building the ruleset, then observe on a fresh account |
